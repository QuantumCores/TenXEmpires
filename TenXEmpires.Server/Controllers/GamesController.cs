...
public class GamesController : ControllerBase
{
    ...
    /// </summary>
    /// <param name="id">The game ID.</param>
    /// <param name="command">The attack command with attacker unit ID and target city ID.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <remarks>
    /// Validations:
    /// - It must be the human player's turn
    ...
    /// <response code="400">Bad request due to invalid payload.</response>
    /// <response code="401">Unauthorized - user is not authenticated.</response>
    /// <response code="404">Not Found - unit or city does not exist or user doesn't have access.</response>
    /// <response code="409">Conflict - NOT_PLAYER_TURN or NO_ACTIONS_LEFT.</response>
    /// <response code="422">Unprocessable Entity - OUT_OF_RANGE or INVALID_TARGET.</response>
    /// <response code="500">Internal server error occurred.</response>
    [HttpPost("{id:long}/actions/attack-city", Name = "AttackCity")]
    [ValidateAntiForgeryTokenApi]
    [ProducesResponseType(typeof(ActionStateResponse), StatusCodes.Status200OK)]
    [SwaggerResponseExample(StatusCodes.Status200OK, typeof(ActionStateResponseExample))]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    ...
    [ProducesResponseType(StatusCodes.Status422UnprocessableEntity)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<ActionStateResponse>> AttackCity(
        long id,
        [FromBody] AttackCityCommand command,
        CancellationToken cancellationToken = default)
    {
        try
        {
            if (!ModelState.IsValid)
            {
                _logger.LogWarning("Invalid request body for AttackCity on game {GameId}", id);
                return BadRequest(new
                {
                    code = "INVALID_INPUT",
                    message = "One or more validation errors occurred.",
            ...
            _logger.LogDebug("User {UserId} attempting city attack in game {GameId} (attacker {AttackerId} -> city {CityId})",
                userId, id, command.AttackerUnitId, command.TargetCityId);

            var response = await _actionService.AttackCityAsync(userId, id, command.AttackerUnitId, command.TargetCityId, idempotencyKey, cancellationToken);
        ...
        {
            _logger.LogWarning(ex, "City attack out of range in game {GameId}", id);
            return UnprocessableEntity(new { code = "OUT_OF_RANGE", message = ex.Message });
        }
        catch (ArgumentException ex) when (ex.Message.Contains("INVALID_TARGET"))
        {
            _logger.LogWarning(ex, "Invalid city attack target in game {GameId}", id);
            return UnprocessableEntity(new { code = "INVALID_TARGET", message = ex.Message });
        }
        catch (InvalidOperationException ex) when (ex.Message.Contains("not found") || ex.Message.Contains("UNIT_NOT_FOUND") || ex.Message.Contains("CITY_NOT_FOUND"))
        {
            _logger.LogWarning(ex, "Unit or city not found or doesn't belong to player in game {GameId}", id);
        ...
        {
            _logger.LogWarning(ex, "Invalid argument for AttackCity request on game {GameId}", id);
            return BadRequest(new { code = "INVALID_INPUT", message = ex.Message });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to execute city attack in game {GameId}", id);
            return StatusCode(
                StatusCodes.Status500InternalServerError,
                new { code = "INTERNAL_ERROR", message = "An error occurred while processing the city attack." });
        }
    }
